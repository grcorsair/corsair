...
> trust.txt discovery supports both `/.well-known/trust.txt` and delegated DNS (TXT or CNAME).
...
**Source tiers (deterministic):** Tool outputs (`tool`, `json`) map to the **tool** tier. Audits and manual evidence (`soc2`, `iso27001`, `pentest`, `manual`) map to the **human** tier.
