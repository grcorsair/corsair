...
Corsair operates like git for compliance. Five core primitives handle signing, verification, transparency, regression tracking, and real-time signals.

```
SIGN → LOG → VERIFY → DIFF → SIGNAL
```

### The Five Primitives (Default Path)
...
### Advanced Features (via `--score` / `--enrich`)

Built on top of the five primitives, these features add intelligence and automation. Activated with `--score`, `--enrich`, or dedicated commands.

| Feature | Command | What It Does |
|---------|---------|--------------|
| **SCORE** | `corsair sign --score` | 7-dimension evidence quality assessment (FICO-style, 0-100) |
| **AUDIT** | `corsair audit` | Multi-source compliance audit with scoring and governance |
| **CERTIFY** | `corsair cert` | Continuous certification lifecycle with drift detection |
| **TPRM** | `corsair tprm` | Vendor risk assessment using CPOEs |

The `--score` flag runs a 7-dimension evidence quality assessment, producing a FICO-style composite score (0-100) with letter grade. Five of the seven dimensions are fully deterministic. Two use model-assisted analysis. Like FICO, it uses models but is not defined by them.
