...
- [Bun](https://bun.sh/) runtime (required to run the CLI)
- Homebrew installs Bun automatically via the `oven-sh/bun` tap; npm install does not
...
- **Hosted API / production:** Postgres required via `DATABASE_URL` for keys, SCITT, and audit trails.
...
# Enforce minimum ingestion contract (issuer/auditor, date, scope)
corsair sign --file example-evidence.json --strict

# Keyless sign via API (OIDC or API key)
corsair sign --file example-evidence.json --auth-token $OIDC_TOKEN --api-url https://api.grcorsair.com
...
corsair verify --domain your-domain.com
```
...
# Delegated DNS (low-friction option):
# _corsair.your-domain.com TXT "corsair-trusttxt=https://trust.your-domain.com/trust.txt"
# _corsair.your-domain.com TXT "corsair-trusttxt-sha256=<sha256>"
# trust.your-domain.com CNAME trust.your-host.com

# Validate from the outside
corsair trust-txt discover your-domain.com --verify
```

Prefer hosted trust.txt? Use the hosted setup wizard at `/setup` or call `POST /trust-txt/host` on the API to get a hosted URL and DNS TXT record.
...
# Diff latest proofs by domain
corsair diff --domain your-domain.com --verify
```
