...
**Mapping registry (tool-agnostic):** For tools not on the list, mappings can extract controls or passthrough fields without code changes. Load mappings via `CORSAIR_MAPPING_DIR`, `CORSAIR_MAPPING_FILE`, the CLI `--mapping` flag, or `corsair mappings add`. Evidence-only mappings still produce valid CPOEs and store passthrough data under `credentialSubject.extensions`. Mappings are evaluated by priority (higher wins), then filename order. Build packs with `corsair mappings pack` and sign them with `corsair mappings sign` — signed packs are verified when `CORSAIR_MAPPING_PACK_PUBKEY` is set.
