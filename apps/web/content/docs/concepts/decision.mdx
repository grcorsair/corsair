...
Layer 3 automates compliance decisions using Layer 1 proofs (signing, verification) and Layer 2 intelligence (normalization, scoring, governance). While Layer 1 creates verifiable evidence and Layer 2 scores it, Layer 3 acts on it: running audits, managing certifications, and assessing vendor risk.

Three engines compose Layer 3:
...
5. Score combined evidence (Layer 2)
6. Run governance checks (optional, Layer 2 Quartermaster)
...
```
Layer 1: Infrastructure (Signing, Verification, SCITT, FLAGSHIP)
    |
    | Signed CPOEs, verified evidence, transparency log
    v
Layer 2: Intelligence (Normalize, Score, Query, Quartermaster)
    |
    | Canonical controls, quality scores, governance findings
    v
Layer 3: Decision (Audit, Certification, TPRM)
    |
...
Each layer adds value without replacing the one below it. A CPOE is verifiable without scoring (Layer 1 alone). A score is meaningful without an audit workflow (Layer 2 alone). But the full stack -- signed evidence, scored quality, automated decisions -- replaces questionnaire-based compliance theater with cryptographically verifiable proof.
