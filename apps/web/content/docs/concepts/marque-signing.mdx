...
Extensions must be namespaced (`mapping`, `passthrough`, `x-`, or `ext.`). `ext.oidc` is reserved for keyless signing delegation metadata and may include **hashed** identity claims when configured.
