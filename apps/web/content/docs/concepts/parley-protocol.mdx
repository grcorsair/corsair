...
2. CLASSIFY  Each control is assigned an assurance level (L0-L4) based on evidence type
3. GENERATE  MARQUE stage produces JWT-VC signed with Ed25519
4. REGISTER  (Optional) JWT-VC submitted to SCITT transparency log
5. SHARE     Vendor shares JWT-VC with requesting organization
6. VERIFY    Requestor verifies locally (Web Crypto, CLI, or any VC verifier)
7. MONITOR   SSF/CAEP stream delivers real-time compliance change events
