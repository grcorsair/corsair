...
Public endpoints (verify, roast, grc translate, health) require no authentication.
...
Scan trust-center pages for a domain (for example `trust.acme.com`) and return a persisted roast report across discoverability, verifiability, freshness, machine readability, and transparency.
The scanner crawls public trust/security/compliance pages, then scores and roasts based on discovered evidence.
Use the returned `fixPreview` and `trustTxtExample` as the handoff into `POST /onboard` or `trust-txt` publishing.
...
| Field | Type | Required | Description |
|:------|:-----|:---------|:-----------|
| `domain` | string | Yes | Domain or URL to scan, e.g. `trust.acme.com` or `https://trust.acme.com` |
    ...
    "pageSignals": [
      {
        ...
        "excerpt": "Security and compliance overview...",
        "linkCount": 22,
        "pdfLinkCount": 4,
        "structuredLinkCount": 2,
        "statusLinkCount": 1,
        "keywordHits": ["security", "compliance", "soc 2"],
