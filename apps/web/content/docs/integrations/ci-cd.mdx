    ...
    steps:
      ...
      - name: Sign scan results into CPOE
        run: |
          ...
          corsair sign \
            --format prowler \
            --file ${{ github.workspace }}/scan-results/prowler-output.json \
            --output /tmp/cpoe.jwt
...
corsair:
  ...
  script:
    - git clone https://github.com/Arudjreis/corsair.git /tmp/corsair
    - cd /tmp/corsair && bun install
    - corsair sign --format prowler --file scan-results/prowler-output.json --output cpoe.jwt
        ...
        stage('Corsair Compliance Signing') {
            steps {
                sh '''
                    corsair sign \
                        --format inspec \
                        --file compliance/inspec-results.json \
...
## Audit and Scoring in CI/CD

Use `corsair audit` to combine multiple evidence files into a single scored audit in your pipeline:

```yaml
      - name: Run compliance audit with scoring
        run: |
          cd /tmp/corsair
          corsair audit \
            --files ${{ github.workspace }}/scan-results/*.json \
            --scope "Release ${{ github.sha }}" \
            --frameworks SOC2,NIST-800-53 \
            --score \
            --json > /tmp/audit-result.json

      - name: Sign with evidence quality score
        run: |
          cd /tmp/corsair
          corsair sign \
            --file ${{ github.workspace }}/scan-results/prowler-output.json \
...
7. **Score evidence quality** — Use `--score` flag to generate FICO-style evidence quality scores alongside signing
8. **Audit multi-source evidence** — Use `corsair audit` to combine multiple tool outputs into a comprehensive compliance audit
9. **Manage certifications** — Use `corsair cert` for ongoing certification management with automatic renewal tracking
