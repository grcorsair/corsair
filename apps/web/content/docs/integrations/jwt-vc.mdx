...
Parley v2 encodes CPOEs as **W3C Verifiable Credentials** in JWT format (`vc+jwt`). This enables interoperability with any standards-compliant VC verifier while maintaining Corsair's Ed25519 signing and evidence chain integrity.
...
```bash
# Run assessment and output as JWT-VC
bun corsair.ts \
  ...
  "vc": {
    ...
    "credentialSubject": {
      ...
      "scope": {
        ...
        "resourceCount": 1,
        "frameworksCovered": ["NIST 800-53", "SOC2"]
      },
      "summary": {
        "controlsTested": 24,
        "controlsPassed": 18,
...
Because Parley v2 uses open standards, CPOEs can be verified by:
