...
For tool outputs not on the built-in list, use the mapping registry on the signing host (see `CORSAIR_MAPPING_DIR` / `CORSAIR_MAPPING_FILE`). Evidence-only mappings still produce a valid CPOE and surface passthrough fields under `extensions`.
