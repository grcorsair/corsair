...
description: "Coming soon. Developer preview only."
...
**Status:** **Coming soon.** The SDK is **not actively maintained** and is **not published to npm**.
If you need a packaged dependency today, use the CLI or API instead.

The `@grcorsair/sdk` package is a typed TypeScript client for programmatic access to Corsair's signing and verification capabilities. It wraps the core engines into a clean API surface with full type exports.

## Installation (Developer Preview)

The SDK is **monorepo-only** for internal development. Clone the repo and use it as a workspace:
...
const result = await corsair.sign(evidence, {
  ...
  strict: false,              // Enforce minimum ingestion contract
...
| `strict` | `boolean` | `false` | Enforce minimum ingestion contract |
