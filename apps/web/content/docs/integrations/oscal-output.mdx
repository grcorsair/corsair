---
title: "OSCAL Output (Coming Soon)"
...
Corsair will generate [OSCAL](https://pages.nist.gov/OSCAL/) (Open Security Controls Assessment Language) Assessment Results — the NIST standard for machine-readable security assessment data.

## Output Formats

```bash
# OSCAL JSON (machine-readable, NIST SP 800-53A compliant)
corsair sign --file tool-output.json --mapping ./mappings/toolx.json --output-format oscal

# HTML report (self-contained, no external dependencies)
corsair sign --file tool-output.json --mapping ./mappings/toolx.json --output-format html

# Markdown report
corsair sign --file tool-output.json --mapping ./mappings/toolx.json --output-format md
```
