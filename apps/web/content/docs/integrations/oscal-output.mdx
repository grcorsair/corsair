...
corsair sign --format prowler --file scan-results.json --output-format oscal

# HTML report (self-contained, no external dependencies)
corsair sign --format inspec --file inspec-results.json --output-format html

# Markdown report
corsair sign --format trivy --file trivy-report.json --output-format md
```
