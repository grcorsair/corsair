...
describe("PgSCITTRegistry.listEntries", () => {
  ...
  test("returns entries with decoded JWT metadata", async () => {
    const { db } = createMockDb();
  ...
  test("handles entries with missing assurance gracefully", async () => {
    const { db } = createMockDb();
    const signingKey = testSigningKey();
    const registry = new PgSCITTRegistry(db, signingKey);

    // JWT without assurance field
...
describe("PgSCITTRegistry.getIssuerProfile", () => {
  ...
  test("returns profile with correct aggregate data", async () => {
    const { db } = createMockDb();
    const signingKey = testSigningKey();
    const registry = new PgSCITTRegistry(db, signingKey);
  ...
  test("handles entries with no assurance or summary", async () => {
    const { db } = createMockDb();
    const signingKey = testSigningKey();
    const registry = new PgSCITTRegistry(db, signingKey);
    ...
    expect(profile!.currentAssuranceLevel).toBe(0);
  });
});
